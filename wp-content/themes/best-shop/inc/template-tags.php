...
if( ! function_exists( 'best_shop_social_links' ) ) :
	...
	*/
    function best_shop_social_links( $echo = true ){
        ...
        $ed_social    = best_shop_get_setting( 'ed_social_links');
							...
							<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="15.498" height="15.498" viewBox="0 0 15.498 15.498" aria-label="Comment Icon">
								<defs>
									<pattern id="pattern" width="1" height="1" patternTransform="matrix(-1, 0, 0, 1, 30.995, 0)" viewBox="0 0 15.498 15.498">
										<image preserveAspectRatio="none" width="15.498" height="15.498" xlink:href="data:image/png;base64,"></image>
									</pattern>
								</defs>
...
function best_shop_breadcrumb(){ 
    ...
    if( best_shop_get_setting( 'enable_breadcrumb', true ) ){
        ...
        if( is_home() ){ 
            ...
            echo $before . '<a itemprop="item" href="'. esc_url( $request_uri ) . '"><span itemprop="name">' . sprintf( __( 'Search Results for "%s"', 'best-shop' ), esc_html( get_search_query() ) ) . '</span></a><meta itemprop="position" content="'. absint( $depth ).'" />' . $after;        
        }elseif( is_day() ){            
            $depth = 2;
            ...
            $depth = 2;
            echo $before . '<a itemprop="item" href="' . esc_url( home_url() ) . '"><span itemprop="name">' . esc_html__( '404 Error - Page Not Found', 'best-shop' ) . '</span></a><meta itemprop="position" content="' . absint( $depth ). '" />' . $after;
        }
...
if( ! function_exists( 'best_shop_single_reading_calc' ) ) :
    ...
    function best_shop_single_reading_calc( $content ){ 
        $ed_read       = best_shop_get_setting( 'enable_post_read_calc' );
        $read_calc     = best_shop_get_setting( 'read_words_per_minute' );
        $clean_content = strip_shortcodes( $content );
        $clean_content = strip_tags( $clean_content );
        $word_count    = str_word_count( $clean_content );
        $time          = ceil( $word_count / $read_calc );
        if ( ! $ed_read ) { ?>
            <div class="time">
                <?php echo absint( $time ) . esc_html__( ' Min Read', 'best-shop' ); ?>
            </div>
        <?php }
    }
endif;
    
if( ! function_exists( 'best_shop_footer_top' ) ) :
    /**
     * Footer Top
    */
    function best_shop_footer_top(){    
...
if( ! function_exists( 'best_shop_footer_menu' ) ) :
    /**
     * Footer Menu
    */
    function best_shop_footer_menu(){ 
